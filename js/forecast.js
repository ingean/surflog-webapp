/* function displayForecasts(forecast, targetId = '', type = 'chart') {
  if (type === 'Table') {displayForecastsTable(forecast,targetId)} else {displayForecastsGraphs(forecast,targetId)};
}

function displayForecastsTable(forecasts,targetId) {
  var table = [];
  for(var i = 0; i <= 50; i++) {
    table.push([]);
  }
  
  const obslocations = Object.keys(locations);

  var rowheaders = [
    '<td class="rowgroup" rowspan="3">B<br>Ø<br>L<br>G<br>E</td><td class="rowsubgroup" rowspan="3">H<br>Ø<br>Y<br>D<br>E</td><td class="rowheader">Sal</td>',
    '<td class="rowheader">Ska</td>',
    '<td class="rowheader">Väd</td>',
    '<td class="rowgroup" rowspan="4">O<br>B<br>S<td class="rowsubgroup" rowspan="4">V<br>Ä<br>D</td><td class="rowheader">Avg</td>',
    '<td class="rowheader">Per</td>',
    '<td class="rowheader">Dir</td>',
    '<td class="rowheader">Max</td>',
    '<td class="rowgroup" rowspan="2">D<br>Ø<br>N<td class="rowsubgroup" rowspan="2">H<br>Ø<br>Y<td class="rowheader">Sal</td>',
    '<td class="rowheader">Ska</td>',
    '<td class="rowgroup" rowspan="4">P<br>E<br>R<br>I<br>O<br>D<br>E</td><td class="rowsubgroup" rowspan="2">B<br>Ø<br>L</td><td class="rowheader">Sal</td>',
    '<td class="rowheader">Ska</td>',
    '<td class="rowsubgroup" rowspan="2">D<br>Ø<br>N<td class="rowheader">Sal</td>',
    '<td class="rowheader">Ska</td>',
    '<td class="rowgroup" rowspan="26">V<br>I<br>N<br>D</td><td class="rowsubgroup" rowspan="2">V<br>A<br>R</td><td class="rowheader">Sal</td>',
    '<td class="rowheader">Ska</td>'];

  for(var i = 0; i < 4; i++) {
    rowheaders.push('<td class="rowgroup" colspan="3"></td>');
  }

  for(var i = 3; i < obslocations.length; i++) {
    rowheaders.push('<td class="rowsubgroup" rowspan="2">' +
                    locations[obslocations[i]].label.substring(0,3) +
                    '</td><td class="rowheader">m/s</td>');
    rowheaders.push('<td class="rowheader">Dir</td>');
  }
  ...
  for (var i = 0; i < forecasts.length; i++){ //Loop through each timestep
    var f = forecasts[i];
    var scope = getScope(f.time.local); //Time
    var colspan = 24 - Number(moment(forecasts[0].time.local).format('HH')); //Timesteps in first day 
    var smhistats = FORECASTSTATS['Saltstein']['smhi']['4'];
    var dmistats = FORECASTSTATS['Saltstein']['dmi']['4'];
    

    //Forecasts
    table[0].push(formatForecast(f.waveheight.forecasts.dmi.saltstein, dmistats, "waveheight", scope));
    table[1].push(formatForecast(f.waveheight.forecasts.dmi.skagerak, dmistats, "swaveheight",scope));
    table[2].push(formatForecast(f.waveheight.forecasts.smhi.avg, smhistats, "waveheight",scope));
    table[3].push(formatForecast(f.waveheight.observations.smhi.avg, smhistats, "waveheight",''));
    table[4].push(formatForecast(f.waveperiod.observations.smhi.avg, smhistats, "waveperiod",''));
    table[5].push(formatWaveDir(f.wavedir.observations.smhi.avg, smhistats, "wavedir",''));
    table[6].push(formatForecast(f.waveheight.observations.smhi.max, smhistats, "waveheightmax",''));
    table[7].push(formatForecast(f.swellheight.forecasts.dmi.saltstein, dmistats, "swellheight",scope));
    table[8].push(formatForecast(f.swellheight.forecasts.dmi.skagerak, dmistats, "sswellheight",scope));
    table[9].push(formatForecast(f.waveperiod.forecasts.dmi.saltstein, dmistats, "waveperiod",scope));
    table[10].push(formatForecast(f.waveperiod.forecasts.dmi.skagerak, dmistats, "swaveperiod",scope));
    table[11].push(formatForecast(f.swellperiod.forecasts.dmi.saltstein, dmistats, "swellperiod",scope));
    table[12].push(formatForecast(f.swellperiod.forecasts.dmi.skagerak, dmistats, "sswellperiod",scope));    
    table[13].push(formatForecast(f.wind.forecasts.dmi.saltstein, dmistats, "wind",scope));          
    table[14].push(formatForecast(f.wind.forecasts.dmi.skagerak, dmistats, "swind",scope));
    
    //Time and score
    table[15].push(formatScore(f.score.dmi.score, scope));            
    table[16].push(formatSurfable(f.surfable.dmi.issurfable, scope)); 
    table[17].push(formatHours(f.time.local));                        
    table[18].push(formatWeekDay(f.time.local, i, colspan));  
    
    var wsr = 19;
    var wdr = 20;

    //Wind observations  
    for(var l = 3 ; l < obslocations.length; l++) {      
      var ws = f.wind.observations.frost[obslocations[l]];
      var wd = f.winddir.observations.frost[obslocations[l]];
      
      table[wsr].push(formatWindObs(ws,wd,obslocations[l],scope));
      table[wdr].push(formatWindDir(wd,ws,obslocations[l],scope));
      wsr += 2;
      wdr += 2;
    }

    //Time and score
    table[47].push(formatScore(f.score.dmi.score, scope));            
    table[48].push(formatSurfable(f.surfable.dmi.issurfable, scope)); 
    table[49].push(formatHours(f.time.local));                        
    table[50].push(formatWeekDay(f.time.local, i, colspan));                                               
  }
  
  var html = '<div class="table-forecast"><table class="table table-condensed">';

  for (var i = 0; i < table.length; i++) {
    html += '<tr>' + rowheaders[i];
    for (var j = 0; j < table[0].length; j++) {
      //html += formatCell(table,i,j);
      html += table[i][j];
    }
    html += '</tr>';
  }
  ...
  html += '</table></div>';
  $('#interpretcontainer' + targetId).html(html);
} */
